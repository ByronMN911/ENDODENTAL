 ...
 * Actualizado con validaciones de negocio para evitar valores negativos.
 */

import models.Producto;
import services.ProductoService;
import services.ProductoServiceImpl;
import services.ServiceJdbcException; // Importante para lanzar errores
...
public class ProductoServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            // Esto protege la base de datos de datos corruptos

            if (precio.compareTo(BigDecimal.ZERO) < 0) {
                throw new ServiceJdbcException("El precio de venta no puede ser negativo.");
            }

            if (stock < 0) {
                throw new ServiceJdbcException("El stock inicial no puede ser negativo.");
            }

            if (stockMinimo < 0) {
                throw new ServiceJdbcException("El stock mínimo no puede ser negativo.");
            }
            ...
            resp.sendRedirect(req.getContextPath() + "/inventario?exito=true");

        } catch (ServiceJdbcException e) {
            // Si hay error de validación (negativos), volvemos con el mensaje
            req.setAttribute("error", e.getMessage());
            // TODO: Podrías reenviar el objeto 'p' para no perder los datos del formulario (Mejora UX)
            doGet(req, resp);
        } catch (Exception e) {
            e.printStackTrace();
